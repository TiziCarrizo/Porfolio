...
class LiquidEther {
  ...
  init() {
    ...
    this.Common = new this.CommonClass(this.container);
    this.Mouse = new this.MouseClass(this.container);
    ...
    this.autoDriver = new this.AutoDriver(this.Mouse, this, {
    ...
    this.output = new this.Output(this);
  ...
  makePaletteTexture(stops) {
    let arr = stops;
    if (!Array.isArray(stops) || stops.length === 0) arr = ['#ffffff', '#ffffff'];
    else if (stops.length === 1) arr = [stops[0], stops[0]];
    const w = arr.length;
  ...
  get Output() {
    ...
    class ShaderPass {
      ...
      update() {
        main.Common.renderer.setRenderTarget(this.props.output || null);
        main.Common.renderer.render(this.scene, this.camera);
        main.Common.renderer.setRenderTarget(null);
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props) {
        const forceX = (main.Mouse.diff.x / 2) * props.mouse_force;
        const forceY = (main.Mouse.diff.y / 2) * props.mouse_force;
        const cursorSizeX = props.cursor_size * props.cellScale.x;
        const cursorSizeY = props.cursor_size * props.cellScale.y;
        const centerX = Math.min(
          Math.max(main.Mouse.coords.x, -1 + cursorSizeX + props.cellScale.x * 2),
          1 - cursorSizeX - props.cellScale.x * 2
        );
        const centerY = Math.min(
          Math.max(main.Mouse.coords.y, -1 + cursorSizeY + props.cellScale.y * 2),
    ...
    class Simulation {
      ...
      createShaderPass() {
        ...
        this.externalForce = new ExternalForce({
          cellScale: this.cellScale, cursor_size: this.options.cursorSize,
          dst: this.fbos.vel_1
        });
      ...
      calcSize() {
        const width = Math.max(1, Math.round(this.options.resolution * main.Common.width));
        const height = Math.max(1, Math.round(this.options.resolution * main.Common.height));
      ...
      update() {
        ...
        this.externalForce.update({ cursor_size: this.options.cursorSize, mouse_force: this.options.mouseForce, cellScale: this.cellScale });
    ...
    return class {
      constructor() {
        this.simulation = new Simulation(main.options);
        this.scene = new THREE.Scene();
        this.camera = new THREE.Camera();
        this.output = new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            uniforms: {
              ...
              palette: { value: main.paletteTex },
              bgColor: { value: main.bgVec4 }
            }
          })
        );
